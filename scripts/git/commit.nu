export-env {
  if 'GIT_COMMIT_TYPE' not-in $env {
    $env.GIT_COMMIT_TYPE = [
      [value    description];
      [feat     'feat:     新增功能 | 新增了一个功能']
      [fix      'fix:      修复缺陷 | 修复了一个 Bug']
      [docs     'docs:     文档更新 | 修改文档，例如修改 README 文件、API 文档等']
      [style    'style:    代码格式 | 修改代码的样式，例如调整缩进、空格、空行等']
      [refactor 'refactor: 代码重构 | 重构代码，例如修改代码结构、变量名、函数名等但不修改功能逻辑']
      [perf     'perf:     性能提升 | 优化性能，例如提升代码的性能、减少内存占用等']
      [test     'test:     测试相关 | 修改测试用例，例如添加、删除、修改代码的测试用例等']
      [build    'build:    构建相关 | 修改项目构建系统，例如修改依赖库、外部接口或者升级 Node 版本等']
      [ci       'ci:       持续集成 | 更改 CI 配置文件或脚本']
      [revert   'revert:   回退代码 | 回退一个或多个更改']
      [chore    'chore:    非业务性 | 对非业务性代码进行修改，例如修改构建流程或者工具配置等']
      [misc     'misc:     暂未归类 | 一些未分类或不知道如何分类的更改']
    ]
  }
